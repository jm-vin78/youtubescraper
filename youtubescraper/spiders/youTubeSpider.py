...
class YoutubespiderSpider(CrawlSpider):
    ...
    def __init__(self, theme='', idSubTopic='', *args, **kwargs):
        super(YoutubespiderSpider, self).__init__(*args, **kwargs)
        self.start_urls = [self.base_url + theme]
        self.theme = theme
        self.idSubTopic = idSubTopic

    def parse_videos(self, response):
        print("Parsing video. Theme: %s" % self.theme)
        ...
        try:
            ...
            sql = ''' INSERT INTO 
                video(name, url, numberOfViews, date, likes, dislikes, description, channelUrl, category, idSubTopic) 
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?) '''
